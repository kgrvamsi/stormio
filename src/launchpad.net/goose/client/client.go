...
var defaultRequiredServiceTypes = []string{"compute", "network", "object-store", "identity", "image"}

func NewClient(creds *identity.Credentials, auth_method identity.AuthMode, requiredServiceTypes []string, serviceUrls identity.ServiceURLs) AuthenticatingClient {
	if nil == requiredServiceTypes {
		defaultRequiredServiceTypes = requiredServiceTypes
	}
	...
	if nil != serviceUrls {
		client.serviceURLs = serviceUrls
	}
	client.auth = &client
	switch auth_method {
	default:
...
func (c *client) sendRequest(method, url, token string, requestData *goosehttp.RequestData) (err error) {
	if (requestData.ReqValue != nil || requestData.RespValue != nil) && !requestData.Binary {
		err = sharedHttpClient.JsonRequest(method, url, token, requestData)
	} else {
		err = sharedHttpClient.BinaryRequest(method, url, token, requestData)
	}
	return
}
...
func (c *authenticatingClient) sendAuthRequest(method, svcType, apiCall string, requestData *goosehttp.RequestData) (err error) {
	if time.Now().After(c.expires) || !c.IsAuthenticated() {
...
func (c *authenticatingClient) doAuthenticate() error {
	...
	c.expires = authDetails.ExpiresAt
	return nil
}
