...
type RunServerOpts struct {
	...
	Personality        []SerializedFile    `json:"personality"`
...
// Rename the server to a given name
func (c *Client) RenameServer(serverId, newName string) (*Entity, error) {
	var req struct {
		Server struct {
			Name string `json:"name"`
		} `json:"server"`
	...
	if err != nil {
		return nil, errors.Newf(err, "failed to rename a server with %s", newName)
	}
	return &resp.Server, nil
}
...
func (c *Client) SetIPV4Address(serverId, address string) error {
	var req struct {
		ServerIPAddressConfig struct {
			AccessIPv4 string `json:"accessIPv4"`
		} `json:"server"`
	}
	req.ServerIPAddressConfig.AccessIPv4 = address
